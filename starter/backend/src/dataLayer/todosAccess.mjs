...
export class TodosAccess {
  ...
  async create(createTodoRequest){
    logger.info(`Creating a todo item with id ${createTodoRequest.todoId} ${JSON.stringify(createTodoRequest, null, 2)}`);
  ...
  async update(userId, todoId, updateTodoRequest = {}) {
    logger.info(`Updating todo item ${todoId} with ${JSON.stringify(updateTodoRequest, null, 2)}`)
  ...
  async setAttachmentUrl(userId, todoId, image, attachmentUrl) {
    logger.info(`Setting attachmentUrl for todo item ${todoId} ${attachmentUrl}`)
    const params = {
      TableName: this.todosTable,
      Key: {
        userId,
  ...
  async delete(userId, todoId) {
    logger.info(`Removing todo item: ${todoId} for user: ${userId}`);
